...
void Hologram::begin()
{
    ...
    //Uncomment to debug over serial
    Serial.begin(115200);
    modem.begin(SerialSystem, *this, &Serial);
...
{
    if (inbound_pending == 0 || modem_state != MODEM_STATE_READY)
        return;
    int readnum = 0;
    int total = 0;
...
bool Hologram::getTime(rtc_datetime_t &dt, bool utc)
{
    ...
    {
        if (dt.year == 4)
        { //good until 2104 and ublox-specfiic
            return false;
        }
        dt.year += 2000;
        if (utc)
...
bool Hologram::writeFile(const char *filename, const char *buffer, const size_t size)
{
    ...
    modem.appendSet(filename);
    modem.appendSet("\",");
    modem.appendSet((int)size);

    if (modem.completeSet(">") == MODEM_OK)
    {
        return modem.command(buffer) == MODEM_OK;
    }
